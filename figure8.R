...
ggplot()+
  ...
  geom_jitter(data = r, mapping = aes(x = type, y = value), width = 0.25, alpha = 0.5 )+
  ylim(0, NA)+
...
ggsave("figures/summary.png", width = 12, height = 12/1.618)
...
ggplot()+
  ...
  geom_jitter(data = r, mapping = aes(x = lc, y = value), width = 0.25, alpha = 0.5 )+
  ylim(0, NA)+
  facet_wrap(facets = vars(name), nrow = 2, scales = "free_y")+
  facet_wrap(~name, 
             nrow = 2, 
...
ggsave("figures/summary_lc.png", width = 12, height = 12/1.618)
